...
class SstParser {
  ...
  /// ادغام SST + CONTINUE و سپس parse.
  void parseMergedSst(List<Uint8List> chunks) {
    ...
    parse(BiffRecord(type: BiffRecordType.sst, data: merged, offset: 0));
  }
  ...
  _StringReadResult _readUnicodeString(Uint8List data, int offset) {
    ...
    final hasPhonetic = (optionFlags & 0x04) != 0;
    final hasRichText = (optionFlags & 0x08) != 0;
    final hasExtSt = (optionFlags & 0x10) != 0;
    ...
    if (pos + richTextRuns * 4 > data.length) {
      pos = data.length;
    } else {
      pos += richTextRuns * 4;
    }

    if (pos + extStringSize > data.length) {
      pos = data.length;
    } else {
      pos += extStringSize;
    }
    ...
    if (pos <= offset) {
      return _StringReadResult(value, 1);
    }
